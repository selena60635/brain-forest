      ...
      <div className="w-1/4">
        ...
        >
          <TabList className="flex justify-between divide-x">
            ...
            <Tab
              className={clsx(
                "grow p-1 data-[selected]:bg-secondary data-[selected]:text-white data-[hover]:bg-primary data-[hover]:text-white data-[selected]:data-[hover]:bg-secondary data-[selected]:data-[hover]:text-white",
                selectedNodes.length === 0 && "pointer-events-none opacity-50"
              )}
            >
              匯出
                  ...
                  <div className="flex flex-wrap justify-between items-center relative">
                    ...
                    <div className="flex">
                      <Menu>
                        <MenuButton className="flex items-center justify-between gap-2 rounded-md border shadow w-20 h-6 px-1 focus:outline-none data-[hover]:bg-gray-100 data-[open]:bg-gray-100 data-[focus]:outline-1 data-[focus]:outline-white ">
                          {
                            borderStyleOpts.find(
                              (option) => option.value === borderStyle
                            )?.icon
                          }
                        ...
                        >
                          {borderStyleOpts.map((option) => (
                            <MenuItem key={option.value}>
                              <button
                                onClick={() => borderStyleChange(option.value)}
                    ...
                    <Menu
                      as="div"
                      className="relative inline-block text-left w-full  mt-4 mb-2"
                    >
                      <MenuButton className="flex items-center justify-between gap-2 rounded-md border shadow w-full h-6 px-2 focus:outline-none data-[hover]:bg-gray-100 data-[open]:bg-gray-100 data-[focus]:outline-1 data-[focus]:outline-white">
                        {
                          borderWidthOpts.find(
                            (option) => option.value === borderWidth
                          )?.icon
                        }
                        <ChevronDownIcon className="size-4 " />
                      </MenuButton>
                      ...
                      <MenuItems
                        transition
                        className="absolute left-0 right-0 origin-top-right rounded-md border shadow-lg bg-white p-1 transition duration-100 ease-out [--anchor-gap:var(--spacing-1)] focus:outline-none data-[closed]:scale-95 data-[closed]:opacity-0"
                      >
                        {borderWidthOpts.map((option) => (
                          <MenuItem key={option.value}>
                            <button
                              onClick={() => borderWidthChange(option.value)}
                              className="group flex items-center gap-2 rounded-lg p-1 w-full data-[focus]:bg-gray-100"
                            >
                              {option.icon}
                            </button>
                          </MenuItem>
